...
def mainGame(movementInfo):
    ...
    while True:
        ...
        if crashTest[0]:
            ...
            idx = len(Qstatelist) -1
            Qvalues[Qstatelist[idx]] = (1 - alpha)*getQvalue(Qstatelist[idx][0],Qstatelist[idx][1]) + alpha * (-1000)
            idx -= 1
            while idx >= 0:
            	nextstate = Qstatelist[idx + 1][0]
            	Qvalues[Qstatelist[idx]] = (1 - alpha)*getQvalue(Qstatelist[idx][0],Qstatelist[idx][1]) + alpha * (1 + discount*computeValueFromQvalues(nextstate))
            	idx -= 1
        ...
        if(flip > episilon):
            ...
            #add to state list
            Qstatelist.append((state,playerFlapped))
